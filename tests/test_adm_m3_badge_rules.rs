...
async fn create_rule_persists_row() {
    ...
    let exists: bool =
        sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM badge_rules WHERE slug = 'test_rule_a')")
            .fetch_one(&app.db)
            .await
            .unwrap();
    assert!(exists);
...
async fn patch_rule_rejects_when_admin_editable_false() {
    ...
    let resp = admin_patch(
        &app,
        "/api/admin/badge-rules/core_rule_x",
        json!({
            "display_name": "Hacked",
        }),
    )
...
async fn deprecate_rule_soft_deletes_and_is_idempotent() {
    ...
    let resp = admin_post(
        &app,
        "/api/admin/badge-rules/to_deprecate_x/deprecate",
        json!({
    ...
    let dep: Option<chrono::DateTime<chrono::Utc>> =
        sqlx::query_scalar("SELECT deprecated_at FROM badge_rules WHERE slug = 'to_deprecate_x'")
            .fetch_one(&app.db)
            .await
            .unwrap();
    assert!(dep.is_some());
