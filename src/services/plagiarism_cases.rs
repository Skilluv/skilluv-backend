...
pub async fn flag(
    ...
    let id = id.ok_or_else(|| {
        AppError::Conflict("a case is already open on this entry".into())
    })?;
...
pub async fn respond(
    ...
    if response.is_empty() {
        return Err(AppError::Validation("an empty answer is not an answer".into()));
    }
...
pub async fn decide(
    ...
    let submission = submission.ok_or_else(|| {
        AppError::Conflict("this case is not open, or does not exist".into())
    })?;
