...
impl DeliverablesService {
    ...
    /// Public so the design validation path (`design_reviews.rs`) can call it:
    /// a validated design deliverable must move the designer's skill graph
    /// exactly as a merged pull request moves a developer's.
