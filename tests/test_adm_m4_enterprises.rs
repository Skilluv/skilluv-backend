...
async fn patch_type_changes_and_resets_config() {
    ...
    let resp = admin_patch(
        &app,
        &format!("/api/admin/enterprises/{ent_id}/type"),
        json!({
    ...
    let (etype, conf): (String, serde_json::Value) =
...
async fn get_type_config_returns_current() {
    ...
    let resp = admin_get(
        &app,
        &format!("/api/admin/enterprises/{ent_id}/type-config"),
    )
    .await;
    assert_eq!(resp.status().as_u16(), 200);
...
async fn agency_clients_returns_empty_for_direct_hire() {
    ...
    let resp = admin_get(
        &app,
        &format!("/api/admin/enterprises/{ent_id}/agency-clients"),
    )
    .await;
    assert_eq!(resp.status().as_u16(), 200);
    let body: serde_json::Value = resp.json().await.unwrap();
