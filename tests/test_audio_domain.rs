...
async fn the_audio_domain_is_active_and_the_undeclared_ones_are_not() {
    ...
    let active: bool =
        sqlx::query_scalar("SELECT is_active FROM skill_domains WHERE slug = 'audio'")
            .fetch_one(&app.db)
            .await
            .unwrap();
...
async fn every_audio_review_capability_is_grantable() {
    ...
    for capability in derived.into_iter().chain([
        "audio_reviewer:all".into(),
        "challenge_validator:audio".into(),
    ]) {
        sqlx::query(
...
async fn the_legacy_game_sound_trade_says_where_it_went() {
    ...
    assert!(
        archived,
        "an orientation nobody should pick is still offered"
    );
    assert_eq!(
...
async fn audio_has_a_default_grid_and_one_per_family() {
    ...
    let default: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM review_grids WHERE domain='audio' AND reviewer_group IS NULL",
    )
    .fetch_one(&app.db)
    .await
    .unwrap();
...
async fn an_audio_slice_says_what_comes_out_of_it() {
    ...
    .await;
    assert!(
        without_subtype.is_err(),
        "an audio artefact with no subtype"
    );
...
async fn the_audio_wizard_publishes_its_own_questions() {
    ...
    let response = app
        .get("/api/users/me/domain-profile/audio/questions")
        .await;
...
async fn the_rank_counts_audio_attestations_like_any_other() {
    ...
    let audio_bases: i64 =
        sqlx::query_scalar("SELECT count(*) FROM attestation_bases WHERE skill_domain = 'audio'")
            .fetch_one(&app.db)
            .await
            .unwrap();
...
/// win and a mentee since migration 0522, and a badge nobody earns without an
...
async fn a_battle_is_won_by_the_first_and_nobody_else() {
    ...
    for (user, expected, who) in [
        (winner, true, "the winner"),
        (second, false, "second place"),
    ] {
...
async fn the_wizard_links_the_handle_it_asked_for() {
    ...
    assert!(
        declared,
        "nothing checked this account, and it should say so"
    );
}
