    ...
    let mentee: Option<Mentee> = sqlx::query_as(
        ...
        "SELECT COALESCE(cs.score, 0) AS craft_score,
                COALESCE(ARRAY(SELECT jsonb_array_elements_text(
                            p.answers -> 'preferred_families')), '{}') AS preferred_families,
                COALESCE(ARRAY(SELECT jsonb_array_elements_text(
...
mod tests {
    ...
    fn no_shared_family_is_not_a_match_at_any_price() {
        assert_eq!(score_candidate(0, 3, 5000, Some(0), 0, CODE.max_active_mentees), 0);
    }

    #[test]
    fn somebody_adjacent_has_nothing_to_teach_yet() {
        assert_eq!(score_candidate(1, 1, 100, Some(0), 0, CODE.max_active_mentees), 0);
    ...
    fn a_full_mentor_is_not_available_whatever_their_calendar_says() {
        assert_eq!(score_candidate(2, 2, 2000, Some(0), CODE.max_active_mentees, CODE.max_active_mentees), 0);
        assert!(score_candidate(2, 2, 2000, Some(0), CODE.max_active_mentees - 1, CODE.max_active_mentees) > 0);
    }

    #[test]
    fn a_shared_language_helps_and_does_not_decide() {
        let same_language_one_family = score_candidate(1, 1, 1000, Some(0), 0, CODE.max_active_mentees);
        let two_families_no_language = score_candidate(2, 0, 1000, Some(0), 0, CODE.max_active_mentees);
        assert!(
    ...
    fn a_full_ai_mentor_is_full_earlier_than_a_code_one() {
        ...
        assert_eq!(score_candidate(2, 2, 2000, Some(0), load, AI.max_active_mentees), 0);
