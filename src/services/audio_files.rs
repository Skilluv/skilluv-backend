...
pub async fn listen_url(
    ...
    storage.presigned_get_url(&key, LISTEN_URL_TTL_SECONDS).await
}
...
pub fn parse_ffprobe(json: &str) -> Measured {
    ...
    let stream = v
        ...
        .and_then(|s| s.iter().find(|s| s.get("codec_type").and_then(|c| c.as_str()) == Some("audio")));
...
pub async fn analyse_pending(
    ...
    for file in waiting {
        ...
        if file.role == "master"
            && let Err(e) = ensure_preview(db, storage, file.id, file.slice_id, &file.storage_key)
                .await
        {
...
pub async fn ensure_preview(
    ...
    if !status.success() {
        return Err(AppError::Internal("ffmpeg could not build a preview".into()));
    }

    let preview_bytes = std::fs::read(out.path())
        .map_err(|e| AppError::Internal(format!("preview read: {e}")))?;
    if preview_bytes.is_empty() {
        return Err(AppError::Internal("ffmpeg produced an empty preview".into()));
    }
...
";
    ...
    fn an_ffmpeg_run_that_said_nothing_measures_nothing() {
        ...
        assert_eq!(parse_ebur128("ffmpeg: command not found"), Measured::default());
    }
}
