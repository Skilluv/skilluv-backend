...
async fn only_code_golf_is_won_at_the_bottom() {
    ...
    let inverted: Vec<String> = sqlx::query_scalar(
        "SELECT slug FROM tournament_kinds WHERE lower_is_better ORDER BY slug",
    )
    .fetch_all(&app.db)
    .await
    .unwrap();
...
async fn a_format_states_what_its_rules_must_carry() {
    ...
    let battle = contest::load_kind(&app.db, "audio_sound_battle").await.unwrap();
