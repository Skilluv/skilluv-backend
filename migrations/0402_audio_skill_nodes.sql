...
-- `ui-sound-design` already exists: migration 0227 gave it to the design
-- domain, under `sound-design-craft`. It is the same skill, and two rows for
-- it would mean a person tagged with one being invisible to a search for the
-- other. The design row stays and this one steps aside — which domain owns a
-- skill both practise is a question for a catalogue review, not for whichever
-- migration happened to run second.
