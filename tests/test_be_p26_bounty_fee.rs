...
async fn platform_revenues_source_enum_accepts_5_values() {
    let (db, name) = setup_test_db().await;
    for source in ["bounty", "mentor_session", "api_metered", "sponsored_challenge", "other"] {
        let res = sqlx::query(
...
async fn platform_revenues_requires_at_least_one_amount() {
    let (db, name) = setup_test_db().await;
    let res = sqlx::query(
        "INSERT INTO platform_revenues (source, fee_rate_bps) VALUES ('bounty', 800)",
    )
    .execute(&db).await;
    assert!(res.is_err(), "row without amount must fail CHECK constraint");
    db.close().await;
    cleanup_test_db(&name).await;
}
...
async fn insert_platform_revenue_bounty_row_and_query_by_enterprise() {
    ...
    .bind(talent_id).execute(&db).await.unwrap();
    ...
    .bind(owner_id).fetch_one(&db).await.unwrap();
    sqlx::query(
    ...
    .bind(slice_id).bind(proj_id).bind(owner_id).execute(&db).await.unwrap();
    ...
    .bind(slice_id).bind(talent_id).bind(ent_id)
    ...
    .bind(ent_id).fetch_one(&db).await.unwrap();
    assert_eq!(count, 1);
