...
async fn a_cohort_attestation_waits_for_all_three_gates() {
    ...
    for (i, learner) in learners.iter().enumerate() {
        record_outcome(&app, cohort, *learner, i == 0).await;
    }
    assert!(issued_for(&app, slice).await.is_empty());

    // All three finished. Recorded on the membership since 0532, which is
    // also what the denominator counts: everybody who joined, not everybody
    // somebody bothered to write a row for.
    sqlx::query("UPDATE cohort_members SET graduated_at = NOW() WHERE cohort_id = $1")
        .bind(cohort)
        .execute(&app.db)
        .await
        .unwrap();
...
async fn a_cohort_is_attested_by_the_person_who_taught_it() {
    ...
    let learners = [
        ...
        user_id(&app, "edul4b").await,
        user_id(&app, "edul4c").await,
    ];
    ...
    for learner in &learners {
        record_outcome(&app, cohort, *learner, true).await;
    }

    let issued = skilluv_backend::services::education_attestations::issue_for_slice(&app.db, slice)
        .await
        .unwrap();
    assert!(
        issued.is_empty(),
...
async fn concluding_a_cohort_says_whether_it_meets_the_threshold() {
    ...
    let response = app
        .post(
            &format!("/api/education/cohorts/{cohort}/conclude"),
            &json!({}),
        )
        .await;
    assert_eq!(response.status(), 200);
...
async fn the_rank_counts_education_attestations_like_any_other() {
    ...
    for _ in 0..11 {
        crate::common::delivered_in(&app, person, "education", "teaching").await;
    }
...
/// Record what happened to one learner, the way the platform does.
///
...
async fn record_outcome(app: &TestApp, cohort: Uuid, learner: Uuid, completed: bool) {
    sqlx::query(
        "INSERT INTO education_learner_outcomes (cohort_id, learner_user_id)
         VALUES ($1, $2)
    ...
    if completed {
        sqlx::query(
            "UPDATE cohort_members SET graduated_at = COALESCE(graduated_at, NOW())
