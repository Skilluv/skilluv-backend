...
async fn audit_admin_role_exists_after_migration_0099() {
    ...
    let exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM pg_roles WHERE rolname = 'audit_admin')",
    )
    .fetch_one(&app.db).await.unwrap();
    assert!(exists, "role audit_admin doit être créé par la migration 0099");
}
...
async fn audit_log_table_has_revoke_documented_via_comment() {
    ...
    let comment: Option<String> = sqlx::query_scalar(
        "SELECT obj_description('admin_audit_log'::regclass, 'pg_class')",
    )
    .fetch_one(&app.db).await.unwrap();
    let text = comment.unwrap_or_default();
    assert!(text.contains("Append-only"), "comment doit documenter le contrat append-only");
}
...
async fn kyc_decide_writes_audit_log() {
    ...
    let ent_id: uuid::Uuid = sqlx::query_scalar(
        "SELECT id FROM enterprises WHERE company_name = 'acmecorp'",
    )
    .fetch_one(&app.db).await.unwrap();
    sqlx::query(
    ...
    let resp = app
        .client
        .post(format!("{}/api/admin/enterprise-kyc/{}/decide", app.addr, ent_id))
    ...
    .bind(admin_uid).bind(ent_id).fetch_one(&app.db).await.unwrap();
...
async fn sso_revoke_writes_audit_log() {
    ...
    let victim_uid: uuid::Uuid = sqlx::query_scalar(
        "SELECT id FROM users WHERE username = 'victim_sso'",
    )
    .fetch_one(&app.db).await.unwrap();
    ...
    .bind(victim_uid).fetch_one(&app.db).await.unwrap();

    app.login("sso_admin").await;
    let resp = app
        .client
        .post(format!("{}/api/admin/sso/sessions/{}/revoke", app.addr, session_id))
        .header("origin", "http://localhost:5174")
        .send().await.unwrap();
    assert_eq!(resp.status().as_u16(), 200);
    ...
    .bind(admin_uid).bind(session_id).fetch_one(&app.db).await.unwrap();
    assert_eq!(audit_count, 1);
}
...
async fn dry_run_env_is_readable_and_default_false() {
    ...
    unsafe { std::env::remove_var("SKILLUV_ADMIN_DRY_RUN"); }
    assert!(!skilluv_backend::middleware::admin_destructive::is_admin_dry_run());
    unsafe { std::env::set_var("SKILLUV_ADMIN_DRY_RUN", "1"); }
    assert!(skilluv_backend::middleware::admin_destructive::is_admin_dry_run());
    // Restore.
    match was {
        Some(v) => unsafe { std::env::set_var("SKILLUV_ADMIN_DRY_RUN", v); },
        None => unsafe { std::env::remove_var("SKILLUV_ADMIN_DRY_RUN"); },
    }
}
