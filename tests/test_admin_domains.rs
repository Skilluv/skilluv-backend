...
async fn a_curator_of_all_reads_every_domain() {
    ...
    for domain in ["design", "code", "ai", "security"] {
        let resp = app
            .get(&format!("/api/admin/domains/{domain}/overview"))
            .await;
...
async fn a_revoked_capability_closes_the_door_again() {
    ...
    assert_eq!(
        app.get("/api/admin/domains/design/overview").await.status(),
        200
    );
...
async fn the_window_is_bounded() {
    ...
    assert_eq!(
        app.get("/api/admin/domains/design/overview?days=0")
            .await
            .status(),
        400
    );
    assert_eq!(
        app.get("/api/admin/domains/design/overview?days=4000")
            .await
            .status(),
        400
    );
...
async fn the_challenge_list_filters_and_counts_honestly() {
    ...
    assert_eq!(
        app.get("/api/admin/challenges?status=whenever")
            .await
            .status(),
        400
    );
...
async fn the_financial_dashboard_reads_the_capability_and_not_the_column() {
    ...
    assert_eq!(
        app.get("/api/admin/dashboard/financial").await.status(),
        200
    );
