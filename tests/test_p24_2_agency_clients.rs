...
async fn create_enterprise(db: &PgPool, slug: &str, ent_type: &str) -> Uuid {
    ...
    .bind(owner).bind(slug).bind(ent_type)
...
async fn insert_ok_for_staffing_agency() {
    ...
    .bind(ent).fetch_one(&db).await.expect("insert ok");
    assert!(!id.is_nil());
    db.close().await;
...
async fn insert_rejected_for_direct_hire() {
    ...
    .bind(ent).execute(&db).await;
    assert!(bad.is_err(), "direct_hire enterprise cannot own agency_clients");
    db.close().await;
    cleanup_test_db(&name).await;
}
...
async fn insert_rejected_for_remote_international() {
    ...
    .bind(ent).execute(&db).await;
    assert!(bad.is_err(), "remote_international enterprise cannot own agency_clients");
    db.close().await;
    cleanup_test_db(&name).await;
}
...
async fn client_name_length_check_enforced() {
    ...
    let bad = sqlx::query(
        "INSERT INTO agency_clients (enterprise_id, client_name) VALUES ($1, 'X')",
    )
    .bind(ent).execute(&db).await;
...
async fn unique_client_name_per_enterprise() {
    ...
    .bind(ent).execute(&db).await.unwrap();
    let dup = sqlx::query(
        "INSERT INTO agency_clients (enterprise_id, client_name) VALUES ($1, 'ClientCo')",
    )
    .bind(ent).execute(&db).await;
...
async fn cascade_delete_when_enterprise_deleted() {
    ...
    sqlx::query("INSERT INTO agency_clients (enterprise_id, client_name)
    ...
    let remaining: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM agency_clients WHERE enterprise_id = $1",
    )
    .bind(ent).fetch_one(&db).await.unwrap();
