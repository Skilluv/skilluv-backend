...
pub async fn list_outcomes(
    ...
    let rows: Vec<OutcomeRow> = sqlx::query_as(
        ...
        SELECT o.learner_user_id, u.username AS learner_username,
               o.pre_assessment, o.post_assessment, o.completed,
...
pub async fn record_outcome(
    ...
    if body.satisfaction.is_some_and(|s| !(1..=5).contains(&s)) {
        return Err(AppError::Validation(
            "satisfaction is one to five".into(),
        ));
    }
    ...
    sqlx::query(
        ...
        INSERT INTO education_learner_outcomes
            (cohort_id, learner_user_id, pre_assessment, post_assessment,
             completed, satisfaction, recorded_by)
        ...
        ON CONFLICT (cohort_id, learner_user_id) DO UPDATE SET
            ...
            completed       = COALESCE($5, education_learner_outcomes.completed),
            satisfaction    = COALESCE($6, education_learner_outcomes.satisfaction),
            recorded_by     = $7,
            updated_at      = NOW()
        "#,
    )
...
pub async fn conclude_cohort(
    ...
    let attestable: bool =
        sqlx::query_scalar("SELECT education_cohort_meets_threshold($1)")
            .bind(cohort_id)
            .fetch_one(&state.db)
            .await?;
...
pub async fn adopt_curriculum(
    ...
    let subtype: Option<String> = sqlx::query_scalar(
        "SELECT education_subtype FROM project_slices WHERE id = $1",
    )
    .bind(slice_id)
    .fetch_optional(&state.db)
    .await?
    ...
    {
        return Err(AppError::Validation(
            "you did not lead that cohort".into(),
        ));
    }
    ...
    if let Err(e) = inserted {
        if e.as_database_error()
            .is_some_and(|db| db.message().contains("not adopted by the person who wrote it"))
        {
            return Err(AppError::Validation(
...
async fn leads_cohort(
    db: &sqlx::PgPool,
    user_id: Uuid,
    cohort_id: Uuid,
) -> Result<bool, AppError> {
    Ok(sqlx::query_scalar::<_, bool>(
        "SELECT led_by_user_id = $2 FROM cohorts WHERE id = $1",
    )
    .bind(cohort_id)
    .bind(user_id)
    .fetch_optional(db)
