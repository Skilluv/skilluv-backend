...
async fn the_legacy_tech_writer_moved_and_was_not_duplicated() {
    ...
    let rows: Vec<(String, String)> = sqlx::query_as(
        "SELECT slug, primary_domain FROM orientations WHERE slug = 'tech-writer'",
    )
    .fetch_all(&app.db)
    .await
    .unwrap();
...
async fn every_communication_badge_can_actually_fire() {
    ...
    assert_eq!(rules.len(), 12, "eleven counted plus one, per migration 0505");
...
async fn an_empty_communication_profile_is_an_apprentice_and_not_a_500() {
    ...
    let response = app
        .get("/api/users/commnewbie/communication-profile")
        .await;
...
async fn a_published_piece_has_to_name_an_address() {
    ...
    for subtype in ["blog_post", "video_content", "devrel_talk", "research_paper"] {
        let refused = sqlx::query(
            "INSERT INTO project_slices
                (project_id, slice_type, title, description, primary_domain, difficulty,
                 communication_subtype)
...
async fn a_translator_cannot_validate_their_own_translation() {
    ...
    sqlx::query(
        "INSERT INTO user_review_languages (user_id, language) VALUES ($1, 'fr')",
    )
    .bind(translator)
    .execute(&app.db)
    .await
...
async fn a_reviewer_signs_only_in_a_language_they_declared() {
    ...
    let issued = skilluv_backend::services::communication_attestations::validate_translation(
        &app.db, reviewer, slice, "sw", "Relu, terminologie cohérente.",
    )
    .await
...
async fn only_a_curator_puts_an_opportunity_on_the_board() {
    ...
    let withdrawn = app
        .delete_with_body(
            &format!("/api/opportunities/{}", listing["data"][0]["id"].as_str().unwrap()),
    ...
    let still_there: i64 =
        sqlx::query_scalar("SELECT count(*) FROM external_opportunities WHERE withdrawn_at IS NOT NULL")
            .fetch_one(&app.db)
            .await
            .unwrap();
    assert_eq!(still_there, 1, "a withdrawn opportunity is kept, not deleted");
}
