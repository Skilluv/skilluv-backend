...
async fn admin_put(app: &TestApp, path: &str, body: serde_json::Value) -> reqwest::Response {
    app.client.put(format!("{}{}", app.addr, path))
        .header("origin", "http://localhost:5174")
        .json(&body).send().await.unwrap()
}
...
async fn recompute_capabilities_returns_report_and_audits() {
    ...
    let resp = admin_post(&app, &format!("/api/admin/users/{target_id}/recompute-capabilities"), json!({})).await;
    ...
    ).bind(admin_uid).bind(target_id).fetch_one(&app.db).await.unwrap();
...
async fn skills_create_persists_row() {
    ...
    let slug = format!("test-sk-{}", &uuid::Uuid::new_v4().simple().to_string()[..8]);
    let resp = admin_post(&app, "/api/admin/skills", json!({
        "slug": slug,
    ...
    let exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM skill_nodes WHERE slug = $1)",
...
async fn skills_update_edits_fields() {
    ...
    let resp = admin_put(&app, &format!("/api/admin/skills/{id}"), json!({
        "display_name": "New Name",
        "is_skilluv_specific": true,
    })).await;
    assert_eq!(resp.status().as_u16(), 200);
    let (name, specific): (String, bool) = sqlx::query_as(
        "SELECT display_name, is_skilluv_specific FROM skill_nodes WHERE id = $1",
