...
///
/// Two gigabytes, which is the adaptive-music budget of migration 0409 —
...
pub fn audio_routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/audio/revisions/{round_id}/resolve", post(resolve_revision))
...
pub async fn list_sources(
    ...
    let declared: Option<Option<chrono::DateTime<chrono::Utc>>> = sqlx::query_scalar(
        "SELECT audio_sources_declared_at FROM project_slices WHERE id = $1",
    )
    .bind(slice_id)
...
pub async fn declare_source(
    ...
    .bind(body.purchase_price_eur.map(bigdecimal::BigDecimal::try_from).transpose().ok().flatten())
