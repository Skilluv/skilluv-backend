...
async fn jsonb_merge_preserves_and_overwrites_keys() {
    ...
    // Set initial config
    sqlx::query(
        "UPDATE enterprises SET type_config = $1::jsonb WHERE id = $2",
    )
    ...
    // Merge patch
    sqlx::query(
        "UPDATE enterprises SET type_config = type_config || $1::jsonb WHERE id = $2",
    )
    .bind(serde_json::json!({"preferred_currency":"EUR","timezone_requirement":"UTC±3"}))
    .bind(e).execute(&db).await.unwrap();

    let cfg: serde_json::Value = sqlx::query_scalar(
        "SELECT type_config FROM enterprises WHERE id = $1",
    )
    .bind(e).fetch_one(&db).await.unwrap();
    assert_eq!(cfg["eor_provider"], "deel", "clé absente du patch préservée");
...
async fn gin_index_supports_jsonb_query() {
    ...
    .bind(e1).execute(&db).await.unwrap();
    sqlx::query(
        "UPDATE enterprises SET type_config = $1::jsonb WHERE id = $2",
    )
    .bind(serde_json::json!({"commission_rate":0.20}))
    .bind(e2).execute(&db).await.unwrap();

    // Query "quelles agences font du white-label ?" via containment
    let ids: Vec<Uuid> = sqlx::query_scalar(
        "SELECT id FROM enterprises WHERE type_config @> $1::jsonb",
    )
