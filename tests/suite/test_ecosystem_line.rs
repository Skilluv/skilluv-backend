...
async fn passing_the_audit_issues_a_dated_badge_and_books_the_fee() {
    ...
    // (95*2 + 80) / 3 = 90
    crate::common::assert_amount(&body["data"]["certification"]["audit_score"], "90.00");
    ...
    .await
    .unwrap();
    crate::common::assert_decimal(&booked, "5000.00");
...
async fn the_creator_sees_their_take_before_listing() {
    ...
    crate::common::assert_amount(&body["data"]["platform_commission"], "4.50");
    crate::common::assert_amount(&body["data"]["creator_receives"], "25.50");
}
...
async fn a_small_item_carries_the_higher_commission() {
    ...
    // its size.
    crate::common::assert_amount(&body["data"]["platform_commission"], "0.60");
    crate::common::assert_amount(&body["data"]["creator_receives"], "2.40");
}
...
async fn a_sale_splits_exactly_and_pays_the_creator() {
    ...
    crate::common::assert_decimal(&split.0, "99.99");
    crate::common::assert_decimal(&split.1, "14.99");
    crate::common::assert_decimal(&split.2, "85.00");
    ...
    .await
    .unwrap();
    crate::common::assert_decimal(&booked, "14.99");
}
...
async fn only_a_buyer_can_rate_and_the_average_follows() {
    ...
    let item: (Option<sqlx::types::BigDecimal>, i32) =
        sqlx::query_as("SELECT rating_avg, rating_count FROM marketplace_items WHERE id = $1")
            ...
            .await
            .unwrap();
    crate::common::assert_decimal(&item.0.unwrap(), "4.00");
    assert_eq!(item.1, 1);
    ...
    let item: (Option<sqlx::types::BigDecimal>, i32) =
        sqlx::query_as("SELECT rating_avg, rating_count FROM marketplace_items WHERE id = $1")
            ...
            .await
            .unwrap();
    crate::common::assert_decimal(&item.0.unwrap(), "2.00");
    assert_eq!(item.1, 1);
}
