...
ALTER TABLE cohorts
    -- NULL for a peer study group, which is every row that existed before
    -- this migration and most rows that will exist after it.
    ADD COLUMN led_by_user_id UUID REFERENCES users(id) ON DELETE SET NULL,
    -- The curriculum this cohort runs, when it runs a published one. What
    -- makes an adoption countable.
    ADD COLUMN curriculum_slice_id UUID REFERENCES project_slices(id) ON DELETE SET NULL,
    -- Closed by the teacher when it has finished, which is what the
    -- attestation generator waits for. A cohort whose end date has passed and
    -- which nobody closed is one that fell apart, and that is worth being
    -- able to tell apart from one that finished.
    ADD COLUMN concluded_at TIMESTAMPTZ;

COMMENT ON COLUMN cohorts.led_by_user_id IS
    'The educator answerable for this cohort, or NULL for a peer study group. '
    'The one column that separates the two, and what the education '
    'attestation generator reads: a cohort with no teacher attests nobody.';

COMMENT ON COLUMN cohorts.concluded_at IS
    'Set by the teacher when the cohort has finished. A cohort past its end '
    'date that nobody concluded is one that fell apart, and that is worth '
    'being able to tell apart from one that ran to the end.';

CREATE INDEX idx_cohorts_taught
