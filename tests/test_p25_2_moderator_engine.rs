...
async fn community_curator_granted_at_three_published_proposals() {
    ...
    assert!(r.granted.contains(&"community_moderator".to_string()),
...
async fn forum_moderator_granted_at_twenty_posts() {
    ...
    let r = capabilities_engine::recompute_capabilities_for_user(&db, u).await.unwrap();
    assert!(!r.granted.contains(&"forum_moderator".to_string()), "19 posts insuffisant");

    add_posts(&db, u, cat, 1).await;
    let r = capabilities_engine::recompute_capabilities_for_user(&db, u).await.unwrap();
    assert!(r.granted.contains(&"forum_moderator".to_string()),
...
async fn plagiarism_and_kyc_reviewer_never_auto_granted() {
    ...
    let r = capabilities_engine::recompute_capabilities_for_user(&db, u).await.unwrap();
    assert!(!r.granted.contains(&"plagiarism_reviewer".to_string()),
        "plagiarism_reviewer manual-only");
    assert!(!r.granted.contains(&"kyc_reviewer".to_string()),
