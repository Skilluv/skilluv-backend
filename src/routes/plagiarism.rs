...
pub async fn flag(
    ...
    if let Err(err) = crate::services::notify::send(
        ...
        crate::services::notify::Recipient::User(
            sqlx::query_scalar::<_, Uuid>(
                "SELECT accused_id FROM plagiarism_cases WHERE id = $1",
            )
            .bind(case.id)
            .fetch_one(&state.db)
            .await?,
    ...
    Ok((axum::http::StatusCode::CREATED, Json(ApiResponse::new(case))))
}
...
pub async fn respond(
    ...
    let case =
        plagiarism_cases::respond(&state.db, id, auth.user_id, &body.response_md).await?;
...
pub async fn decide(
    ...
    let case = plagiarism_cases::decide(
        &state.db,
        id,
        auth.user_id,
        body.upheld,
