...
async fn public_get_returns_active_capabilities() {
    ...
    let caps: Vec<String> = body["data"]["capabilities"].as_array().unwrap()
        .iter().map(|c| c["capability"].as_str().unwrap().into()).collect();
...
async fn me_capabilities_requires_auth_and_returns_own() {
    ...
    let uid: uuid::Uuid = sqlx::query_scalar("SELECT id FROM users WHERE username = 'kim184b'")
        .fetch_one(&app.db).await.unwrap();
    sqlx::query("INSERT INTO user_capabilities (user_id, capability) VALUES ($1, 'issue_proposer')")
        .bind(uid).execute(&app.db).await.unwrap();
...
async fn admin_can_grant_capability_to_other_user() {
    ...
    let target_id: uuid::Uuid = sqlx::query_scalar(
        "SELECT id FROM users WHERE username = 'target184c'",
    ).fetch_one(&app.db).await.unwrap();
...
async fn non_admin_cannot_grant() {
    ...
    let target_id: uuid::Uuid = sqlx::query_scalar(
        "SELECT id FROM users WHERE username = 'target184d'",
    ).fetch_one(&app.db).await.unwrap();
...
async fn admin_can_revoke_active_capability() {
    ...
    let target_id: uuid::Uuid = sqlx::query_scalar(
        "SELECT id FROM users WHERE username = 'target184e'",
    ).fetch_one(&app.db).await.unwrap();
    sqlx::query("INSERT INTO user_capabilities (user_id, capability) VALUES ($1, 'mentor')")
        .bind(target_id).execute(&app.db).await.unwrap();

    app.login("adm184e").await;
    let resp = app.delete(&format!("/api/admin/users/{target_id}/capabilities/mentor")).await;
    assert_eq!(resp.status().as_u16(), 200);
...
async fn revoke_404_on_unknown_capability() {
    ...
    let target_id: uuid::Uuid = sqlx::query_scalar(
        "SELECT id FROM users WHERE username = 'target184f'",
    ).fetch_one(&app.db).await.unwrap();
