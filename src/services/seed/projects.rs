//! The repositories Skilluv draws work from, wired to the P11 GitHub ingestor.
//!
//! Five catalogues: our own repositories, where dogfooding happens; the partner
//! repositories of Annexe F, curated unilaterally -- no permission is needed to
//! point people at a public issue tracker; and the large ecosystem, AI and ops
//! projects, which take contributions in every trade.
//!
//! Labels are per repository. Rust projects say `E-easy`, most say `good first
//! issue`, and our own say `skilluv-challenge`; one global list only ever
//! fitted the last of those.
//!
//! ## Why the catalogue lives here rather than in the binary
//!
//! It is platform data, not a command-line tool's private constant. The
//! deployment seeds itself now, so the list has to be reachable from the
//! server; `bin/seed_projects.rs` is a thin wrapper over this.
//!
//! Idempotent: `INSERT ... ON CONFLICT (slug) DO UPDATE` refreshes
//! `curated_labels` and `slice_ingestion_mode` on every re-run.
...
/// Seed every catalogue. Returns what it did, for the ledger.
pub async fn run(db: &PgPool, owner_id: Uuid) -> Result<String, AppError> {
    let mut created = 0usize;
    let mut updated = 0usize;

    for repo in ALL_PROJECTS.iter().flat_map(|c| c.iter()) {
    ...
    Ok(format!("{created} created, {updated} updated"))
}
