...
async fn sweep_dry_run_returns_count_without_running_engine() {
    ...
    let resp = admin_post(&app, "/api/admin/proof-hooks/sweep?within_days=30&dry_run=true", json!({})).await;
...
async fn admin_gdpr_export_queues_and_audits() {
    ...
    let resp = admin_post(&app, &format!("/api/admin/users/{target_id}/gdpr-export"), json!({
