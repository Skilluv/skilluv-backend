...
//!
//! ## Why a binary and not a migration
//!
//! A slice needs a project, and a project needs an owner. A migration has no
//! owner to give it: on a fresh database there is no admin yet, so a migration
//! would either invent a user or silently seed nothing. `seed_projects`
//! already solved this by taking an owner and being re-runnable, and this
//! follows it.
//!
...
//! claimed is not yanked back to `open` by a re-run.
...
struct Cli {
    /// Email of the admin the seeded projects belong to.
...
async fn main() -> Result<()> {
    ...
    for brief in BRIEFS {
        let project: Option<(Uuid,)> = sqlx::query_as("SELECT id FROM projects WHERE slug = $1")
            ...
            .fetch_optional(&db)
            .await
            .with_context(|| format!("failed to look up project {}", brief.project_slug))?;
        ...
        let Some((project_id,)) = project else {
            tracing::warn!(
                ...
                title = brief.title,
                "project not seeded yet — run `cargo run --bin skilluv-seed-projects` first"
            );
            skipped += 1;
            continue;
        };
        ...
        .fetch_optional(&db)
        .await
        .with_context(|| format!("failed to look up orientation {}", brief.orientation))?;
        ...
        let existing: Option<(Uuid,)> = sqlx::query_as(
            "SELECT id FROM project_slices WHERE project_id = $1 AND title = $2",
        )
        .bind(project_id)
        .bind(brief.title)
        .fetch_optional(&db)
        .await
        .context("failed to look for an existing slice")?;
        ...
        match existing {
            Some((id,)) => {
                ...
                .execute(&db)
                .await
                .with_context(|| format!("failed to update {}", brief.title))?;
                updated += 1;
            ...
            None => {
                ...
                .execute(&db)
                .await
                .with_context(|| format!("failed to insert {}", brief.title))?;
                created += 1;
    ...
    tracing::info!(
        ...
        updated,
        skipped,
        contests_recommended = contests,
        "design canvas seeded"
    );

    if skipped > 0 {
        // A non-zero exit, because a partially seeded canvas that reports
        // success is the kind of thing somebody discovers a month later.
        anyhow::bail!("{skipped} briefs had no project to land in");
    }
    Ok(())
}
