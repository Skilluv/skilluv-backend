...
async fn an_approved_loan_books_its_origination_commission() {
    ...
    assert_eq!(resp.status(), 200, "{}", resp.text().await.unwrap());
    let body: Value = resp.json().await.unwrap();
    crate::common::assert_amount(&body["data"]["commission"], "240.00");
}
...
async fn the_net_amount_is_stated_before_anybody_agrees() {
    ...
    crate::common::assert_amount(&body["data"]["advance"]["advance_amount"], "500.00");
    crate::common::assert_amount(&body["data"]["you_would_receive"], "480.00");
}
...
async fn disbursing_pays_the_net_and_books_the_fee() {
    ...
    crate::common::assert_amount(&body["data"]["paid_out"], "576.00");
    ...
    .await
    .unwrap();
    crate::common::assert_decimal(&booked, "24.00");
}
...
async fn a_claim_is_capped_per_mission_and_per_year() {
    ...
    assert_eq!(resp.status(), 200, "{}", resp.text().await.unwrap());
    let body: Value = resp.json().await.unwrap();
    crate::common::assert_amount(&body["data"]["paid"], "500.00");
