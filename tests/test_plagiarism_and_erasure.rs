...
async fn the_accused_reads_the_case_and_answers_it() {
    ...
    assert_eq!(
        app.get(&format!("/api/contests/plagiarism/{case}")).await.status(),
        403
    );
    ...
    assert_eq!(answered["data"]["status"], "open", "answering decides nothing");
}
...
async fn an_upheld_case_marks_the_entry_and_never_deletes_it() {
    ...
    let (status, notes): (String, Option<String>) = sqlx::query_as(
        "SELECT status, judge_notes FROM tournament_submissions WHERE id = $1",
    )
    .bind(entry)
    .fetch_one(&app.db)
    .await
...
async fn erasure_leaves_a_tombstone_with_nothing_personal_on_it() {
    ...
    assert!(email.ends_with("@invalid"), "a stray mailer must reach nobody");
    assert!(bio.is_none());
...
async fn erasure_keeps_what_other_people_rely_on() {
    ...
    let survives: bool = sqlx::query_scalar(
        "SELECT EXISTS (SELECT 1 FROM tournament_submissions WHERE id = $1)",
    )
    .bind(entry)
    .fetch_one(&app.db)
    .await
...
async fn erasure_takes_the_tokens_with_it() {
    ...
    let left: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM design_cloud_connections WHERE user_id = $1",
    )
    .bind(user)
    .fetch_one(&app.db)
    .await
...
async fn erasing_twice_changes_nothing() {
    ...
    assert!(skilluv_backend::services::erasure::erase(&app.db, user).await.unwrap());
    let name_after_first: String =
