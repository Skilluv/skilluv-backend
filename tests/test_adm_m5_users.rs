...
async fn recompute_proofs_returns_report_and_audits() {
    ...
    let resp = admin_post(&app, &format!("/api/admin/users/{target_id}/recompute-proofs?dry_run=true"), json!({
        "scope": "all",
...
async fn rank_override_writes_row_and_updates_user_ranks() {
    ...
    let resp = admin_post(&app, &format!("/api/admin/users/{target_id}/rank-override"), json!({
        "new_rank": "maitre",
    ...
    ).bind(target_id).bind(admin_uid).fetch_one(&app.db).await.unwrap();
...
async fn peek_user_orientations_admin_scoped() {
    ...
    let orient_id: uuid::Uuid = sqlx::query_scalar(
    ...
    ).bind(target_id).bind(orient_id).execute(&app.db).await.unwrap();
