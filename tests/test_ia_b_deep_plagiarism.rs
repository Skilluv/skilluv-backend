...
async fn create_deliverable(app: &TestApp, code: Option<&str>) -> uuid::Uuid {
    ...
    .bind(cid).bind(user_id).bind(meta).fetch_one(&app.db).await.unwrap()
}
...
async fn deep_scan_route_rejects_non_admin_non_reviewer() {
    ...
    let uid: uuid::Uuid = sqlx::query_scalar("SELECT id FROM users WHERE username = 'plain_user_dp'")
        .fetch_one(&app.db).await.unwrap();
    ...
    .bind(uid).bind(format!("c-{uid}").into_bytes()).execute(&app.db).await.unwrap();
    ...
    let resp = app
        ...
        .send().await.unwrap();
    assert_eq!(resp.status().as_u16(), 403, "plain user must be refused (no admin nor plagiarism_reviewer)");
}
...
async fn deep_scan_route_rejects_admin_without_ai_client() {
    ...
    assert!(matches!(s, 500 | 400), "admin passe le capability gate, échoue plus loin (statut vu: {s})");
}
...
async fn deep_scan_route_rejects_empty_code_content() {
    ...
    let resp = app
        ...
        .header("origin", "http://localhost:5174")
        .send().await.unwrap();
    assert_eq!(resp.status().as_u16(), 400, "code_content vide → 400 validation");
}
...
async fn plagiarism_reviewer_capability_grants_access() {
    ...
    let s = resp.status().as_u16();
    assert!(matches!(s, 500 | 400), "plagiarism_reviewer passe le gate (statut vu: {s})");
}
