...
async fn a_published_brief_becomes_something_somebody_can_claim() {
    ...
    let fragments_before: i32 = sqlx::query_scalar("SELECT total_fragments FROM users WHERE id = $1")
        .bind(author)
        .fetch_one(&app.db)
        .await
        .unwrap();
    ...
    let published: Value = app
        .post(&format!("/api/admin/design/briefs/{id}/publish"), &json!({}))
        .await
        .json()
        .await
        .unwrap();
    ...
    let fragments_after: i32 = sqlx::query_scalar("SELECT total_fragments FROM users WHERE id = $1")
        .bind(author)
        .fetch_one(&app.db)
        .await
        .unwrap();
